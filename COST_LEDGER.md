...
2026-08-31T07:52Z | item H1 RECORDED and MEASURED: `group_repeated` pointed at `group_chat`, agent arm only. Baseline and ablation were cache HITS and cost nothing — verified before the run with 22 replay hits and 0 misses, and confirmed after by both reproducing their published figures exactly. The arm LOST on both adopt criteria and the code was reverted; the 24 entries are kept committed so `git apply experiments/h1-group-chat.patch && make eval` reproduces the loss with no key | calls=24 | in_tok=44765 | out_tok=4825 | est_usd=0.0064 | running_total=0.4364
